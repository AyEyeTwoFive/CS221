...
def mutate_sentences(sentence: str) -> List[str]:
    ...
    for i in range(len(words)-1):
        next[words[i]].add(words[i+1])

    def find_similar(sentence):
        if len(words)==len(sentence):
            sim.append(' '.join(sentence))
        else:
            for next_word in next[sentence[-1]]:
                find_similar(sentence + [next_word])
...
def find_nonsingleton_words(text: str) -> Set[str]:
    ...
    return set(w for w in counts if counts[w] > 1)
    # END_YOUR_CODE
